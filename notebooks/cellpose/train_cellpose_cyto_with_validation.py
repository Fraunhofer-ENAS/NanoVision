...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fine-tune the classic Cellpose cyto model with separate "
...
def save_loss_history(
    *,
    ...
    with plt.style.context(
        ...
        ax.set_title(
            "Classic Cellpose cyto fine-tuning"
        )
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is not available in .venv_cellpose_original."
        )
