...
def train_seg(
    ...
    best_filename = Path(
        f"{filename}_best_validation"
    )

    best_record_path = Path(
        f"{filename}_best_validation.txt"
    )
    ...
    if early_stopping_min_delta < 0:
        raise ValueError(
            "early_stopping_min_delta must be nonnegative."
        )
    ...
    for iepoch in range(n_epochs):
        ...
        # Evaluate and log training/validation loss after every epoch.
        if True:
            lavgt = 0.
            if test_data is not None or test_files is not None:
                ...
                if validation_improved:
                    best_validation_loss = float(lavgt)
                    best_epoch = int(iepoch)
                    epochs_without_improvement = 0
                    ...
                    train_logger.info(
                        ...
                        f"epoch={iepoch}, "
                        f"loss={lavgt:.8f}; "
                        f"saving to {best_filename}"
                    )

                    net.save_model(best_filename)

                    best_record_path.write_text(
                        (
                            f"best_epoch_zero_based={best_epoch}\n"
                            f"best_epoch_one_based={best_epoch + 1}\n"
                            f"best_validation_loss="
                            f"{best_validation_loss:.12f}\n"
                            f"best_model={best_filename}\n"
                        ),
                        encoding="utf-8",
                    )
                else:
    ...
    completed_epoch_count = last_completed_epoch + 1
