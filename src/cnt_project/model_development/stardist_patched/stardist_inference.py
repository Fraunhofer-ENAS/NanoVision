...
from csbdeep.utils.tf import keras_import, IS_TF_1, CARETensorBoard, BACKEND as K, export_SavedModel
...
from csbdeep.utils import _raise, axes_check_and_normalize, axes_dict, load_json, save_json, backend_channels_last
...
from stardist.nms import _ind_prob_thresh
from stardist.utils import _is_power_of_2, _is_floatarray, optimize_threshold, grid_divisible_patch_size
...
# Custom StarDist Base Model (Inference and Prediction)
class MyStarDistBase(BaseModel):
    ...
    def predict_raw_points(
        ...
        *,
        ...
        """
        Return raw model candidate points before NMS and before polygon merging.

        Returns
        -------
        dict with:
            scores: ndarray (N,)
            dist: ndarray (N, n_rays)
            points: ndarray (N, 2), y/x coordinates in image space
        """
        if predict_kwargs is None:
            predict_kwargs = {}

        if self._is_multiclass():
            scores, dist, prob_class, points = self.predict_sparse(
                img,
        ...
        scores, dist, points = self.predict_sparse(
            img,
            prob_thresh=prob_thresh,
            axes=axes,
            normalizer=normalizer,
    ...
    def _predict_instances_generator(self, img, axes=None, normalizer=None,
                                     ...
                                     overlap_label=None, return_predict=False, fname =None):
        ...
        res_instances = self._instances_from_prediction(_shape_inst, prob, dist,
                                                        ...
                                                        overlap_label=overlap_label, fname = fname, 
                                                        **nms_kwargs)
    ...
    def predict_instances(self ,*args,  fname= None, **kwargs):
        ...
        # return last "yield"ed value of generator
        # Route postprocessing-debug kwargs into nms_kwargs so callers can pass
        # them directly via predict_instances(...).
        debug_keys = ("debug_plots", "debug_plot_dir", "debug_run_name", "debug_plot_level")
        extracted_debug = {k: kwargs.pop(k) for k in debug_keys if k in kwargs}
        if extracted_debug:
            ...
            nms_kwargs.update(extracted_debug)
            kwargs["nms_kwargs"] = nms_kwargs

        r = None
        for r in self._predict_instances_generator(*args, fname=fname , **kwargs):
            pass
        return r
