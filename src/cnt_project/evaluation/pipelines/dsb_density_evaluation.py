...
from cnt_project.evaluation.inputs.evaluation_grouping import (
    load_density_groups,
    normalize_density_selection,
)
from cnt_project.evaluation.utils import safe_progress_iter

def run_dsb_by_density(
    selected_density: str | None = None,
    *,
    ...
    if not Path(gt_json_path).exists():
        raise FileNotFoundError( f"Ground-truth COCO JSON does not exist: {gt_json_path}" )
    
    selected_density = normalize_density_selection(selected_density)
    ...
    (Path(out_dir) / "dsb_density_metadata.json").write_text(
    ...
    if selected_density == "ALL":
        groups_to_run = ["LOW_DENSITY", "MID_DENSITY", "HIGH_DENSITY"]
    else:
        groups_to_run = [selected_density]

    missing = [g for g in groups_to_run if g not in density_groups]
    if missing:
        raise ValueError(
            f"Density group(s) {missing} not available. "
            f"Available: {sorted(density_groups.keys())}"
        )

    for density_key in groups_to_run:
        target_filenames = density_groups[density_key]
        ...
        # write into out_dir (instead of results_dir)
        ...
        df_summary.to_csv(os.path.join(out_dir, f"ap_results_poly_{density_key}.csv"), index=False)

        df_per_image = pd.DataFrame(per_image_results)
        df_per_image.to_csv(os.path.join(out_dir, f"ap_dice_results_poly_{density_key}.csv"), index=False)
