...
from cnt_project.model_development.postprocessing.geometry.polygon_from_rays import (
...
def raw_candidates_to_polygon_binary_mask(
    *,
    points: np.ndarray,
    scores: np.ndarray,
    dist: np.ndarray,
    ...
    """
    Convert raw pre-NMS StarDist candidates into a dense binary mask.

    Each candidate point + ray distances is converted to a polygon.
    All polygons with score > score_threshold are filled into one binary mask.
    """
    h, w = image_shape[:2]
    mask = np.zeros((h, w), dtype=np.uint8)

    if points is None or scores is None or dist is None:
        return mask
    ...
    if points.size == 0 or scores.size == 0 or dist.size == 0:
        return mask
    ...
    for i in keep_indices:
        try:
            poly, _, _, _ = construct_polygon_opp(i, dist, points, scores)
        except Exception as exc:
            print(f"Skipping raw candidate {i}: polygon construction failed: {exc}")
            continue
...
def run_pre_nms_polygon_inference(
    *,
    loader: CNTDataset,
    model_name: str,
    basedir: str,
    ...
    """
    Run model inference and return dense binary masks made from raw pre-NMS polygons.
    """
    ...
    for i, sample in enumerate(samples):
        print(f"[{model_name}] pre-NMS polygon inference {i + 1}/{len(samples)} - {sample.X_fn}")
        ...
        pred_mask = raw_candidates_to_polygon_binary_mask(
            points=raw["points"],
            scores=raw["scores"],
            dist=raw["dist"],
...
def run_pre_nms_polygon_metrics_by_density(
    *,
    ...
    """
    Evaluate dense binary masks reconstructed from raw pre-NMS polygons by density.

    Outputs per model/density:
      global_outputs/runs/ <MODEL>__pre-nms-polygon-density-eval__<TIMESTAMP>/ eval/pre_nms_polygon_masks/density_eval/<DENSITY>/

    Aggregated outputs:
      global_outputs/reports/<REPORT>/pre_nms_polygon_density_comparison/
    """
    ...
    for model_name in model_names:
        evaluation_run_id = make_run_id( model_name, "pre-nms-polygon-density-eval", ts=evaluation_timestamp, )
        ...
        pred_masks = run_pre_nms_polygon_inference(
            loader=dataset,
            model_name=model_name,
            basedir=basedir,
            score_threshold=score_threshold,
        ...
        for density in selected_densities:
            out_dir = ( run_paths.eval_dir / "pre_nms_polygon_masks" / "density_eval" / density.lower() )
            ...
            print( f"[{model_name}] {density}: " f"evaluating {len(subset_samples)} pre-NMS polygon masks..." )
        ...
        print( f"[{model_name}] outputs: " f"{run_paths.eval_dir / 'pre_nms_polygon_masks' / 'density_eval'}" )

    if report_name is None:
        report_name = f"pre_nms_polygon_density_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    report_dir = P.report_dir(report_name, "pre_nms_polygon_density_comparison")
    ...
    print(f"Saved pre-NMS polygon density comparison outputs to: {report_dir}")
    return return_path
